...
> 一站式 Harmony Next 应用开发辅助工具：投屏控制、性能监控、UI 调试、应用重签名
...
| Linux   | `.AppImage` / `.deb` | 根据发行版选择对应格式 |
...
- **HDC 工具**：系统中已安装 HDC（Harmony Device Connector），并已添加到系统 PATH 环境变量
  - HDC 通常随 DevEco Studio 一起安装，路径类似：`DevEco Studio/sdk/toolchains/hdc`
  - 可在终端执行 `hdc version` 验证是否可用
- **设备连接**：
...
> 如果你只使用「投屏」和「性能测试」功能，无需登录账号。

---

## 三、功能介绍与使用说明

### 3.1 设备投屏（内置 UI Viewer）
...
- 内置 UI Viewer：获取当前界面的 UI 树结构，在画面上 Hover / 点击联动高亮控件

**使用步骤**：
1. 在左侧导航栏点击「设备」，进入设备列表页面
...
4. 在弹出的投屏窗口中：
   ...
   - 点击工具栏的「UI Viewer」按钮打开 UI 结构面板，获取并分析当前界面控件
