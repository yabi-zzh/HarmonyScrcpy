...
> 一站式 HarmonyOS NEXT 应用开发辅助工具：设备投屏、截图录屏、性能监控、应用解析、应用重签名

---

## 一、项目简介

HO-Kit 是一款面向 HarmonyOS NEXT 应用开发者的桌面工具，将投屏、截图录屏、性能监控、UI 结构分析、应用重签名等高频开发场景整合到统一界面中，帮助开发者提升调试效率、减少工具切换成本。
...
> **注意**：如果 Releases 页面暂时没有安装包，说明项目还在开发中，请参考下方「从源码运行」小节自行构建。

> **macOS 用户注意**：首次打开应用时，如果提示"应用已损坏"或"无法验证开发者"，请在终端执行以下命令后重试：
> ```bash
...
使用 HO-Kit 的大部分功能需要连接 HarmonyOS NEXT 设备，请确保：

- **HDC 工具**：HO-Kit 已内置 HDC（Harmony Device Connector），无需额外配置
  - 如果系统 PATH 中已配置 HDC，优先使用系统版本
  - 如果系统未配置，自动使用内置版本
- **设备连接**：
  - **USB 连接**：使用数据线将 HarmonyOS NEXT 设备连接到电脑，并在设备上开启「开发者选项」和「USB 调试」
...
> **无需登录的功能**：设备投屏、性能监控、应用解析等功能都是纯本地操作，无需登录华为账号即可使用。
...
将 HarmonyOS NEXT 设备的屏幕实时投射到电脑上，并支持远程操作和 UI 结构分析。

**功能特性**：
- 支持 JPEG 与 H.264 两种投屏模式，可根据网络和性能情况灵活选择
- 支持 USB 有线连接和 WiFi 无线连接
- 投屏窗口支持：缩放、双击适配画面、屏幕旋转、窗口置顶、屏幕常亮
- 支持远程按键操作：返回键、Home 键、多任务键、电源键
- 支持鼠标快捷键：中键 HOME、右键返回、侧键多任务/通知栏
- 支持截图和录屏功能，可自定义保存路径
...
- **同应用多包签名顺序**：如果同一个应用有多个 HAP/HSP 包需要分开签名时，请务必先签名 HAP 主包，再签名 HSP 共享包，否则可能出现 ACL 权限缺失问题
...
在投屏时启用额外的扩展功能，增强调试体验。

**前置条件**：
- 需要在手机上安装 HoKit 服务应用（包名：`com.hokit.app`）
- HAP 安装包位于：`assets/tools/hap/ho-kit.hap`
...
| 功能 | 说明 |
|-----|------|
| 输入法服务 | 使用电脑键盘直接在手机上输入中英文，支持拼音输入和常用快捷键（Ctrl+A/C/V/Z 等） |
| 音频服务 | 将手机音频实时同步到电脑播放，实现投屏时的音画同步 |

**使用步骤**：
1. 开始投屏后，点击投屏窗口导航栏的「扩展」按钮
2. 在弹出的扩展功能窗口中，点击「启动服务」
3. **输入法服务**：
   - 启动服务后，点击「切换输入法」（HarmonyOS 6.0+）或手动在设备设置中切换到 HoKit 输入法
   - 连接成功后，即可使用电脑键盘在手机上输入
4. **音频服务**：
   - 启动服务后，系统会弹出「屏幕隐私保护」权限弹窗，工具会自动点击授权
   - 连接成功后，手机播放的音频会同步输出到电脑

**注意事项**：
- 扩展功能需要 HoKit 服务应用已安装且签名有效
- 输入法切换方式因系统版本不同：HarmonyOS 6.0+ 支持命令行切换，5.0 需手动操作
- **首次输入时，第一个字母会被忽略**，这是用于触发手机端输入法激活，属于正常行为
- 停止投屏时会自动断开扩展服务连接
...
| 限制项 | 说明 |
|-------|------|
| 操作系统 | 支持 Windows 和 macOS（含 Apple Silicon），Linux 可能存在兼容性问题 |
| 设备类型 | 仅支持 HarmonyOS NEXT 系统设备（手机、平板等） |
...
- 这是 macOS 的安全机制导致的，在终端执行以下命令后重试：
  ```bash
  sudo xattr -rd com.apple.quarantine /Applications/HO-Kit.app
  ```
